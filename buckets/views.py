...
class BucketObjectsView(APIView):
    ...
    def get(self, request, bucket_id):
        ...
        perm = get_object_or_404(BucketPermission, user=request.user, bucket=bucket, can_view=True)

        try:
            objects = aws_client.list_objects_v2(Bucket=bucket.name, Prefix=bucket.prefix or '')
...
class UploadObjectView(APIView):
    ...
    def post(self, request, bucket_id):
        bucket = get_object_or_404(Bucket, id=bucket_id)
        perm = get_object_or_404(BucketPermission, user=request.user, bucket=bucket, can_upload=True)
...
class DeleteObjectView(APIView):
    ...
    def delete(self, request, bucket_id, key):
        bucket = get_object_or_404(Bucket, id=bucket_id)
        perm = get_object_or_404(BucketPermission, user=request.user, bucket=bucket, can_delete=True)
